      ...
      <Link to={`/projects/${project.id}`} className="flex h-full min-w-0 flex-col">
        ...
        <div className="flex min-w-0 flex-1 flex-col px-3 pb-3 pt-5">
          <div className="mb-4 flex items-center justify-between gap-4 border-b border-line/70 pb-3">
            <p className="line-clamp-1 min-w-0 text-[10px] uppercase leading-5 tracking-[0.16em] text-primary">
              {project.type}
            </p>
            <span className="shrink-0 text-xs text-ink/42">{project.year}</span>
          </div>
          <h3 className="break-words text-xl font-medium leading-8 text-ink">{project.title}</h3>
          <p className="mt-2 line-clamp-2 min-h-11 break-words text-[13px] leading-[1.7] text-ink/56">
          ...
          <div className="mt-5 flex min-h-[4.25rem] flex-wrap content-start gap-2">
            {project.keywords.slice(0, 4).map((keyword) => (
              <span
                key={keyword}
                className="max-w-full break-words rounded-full border border-line bg-transparent px-3 py-1 text-[11px] text-ink/58"
              >
                {keyword}
              </span>
            ))}
